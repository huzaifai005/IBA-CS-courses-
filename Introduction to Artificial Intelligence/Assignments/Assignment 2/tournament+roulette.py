...
def main():
    ...
    for gen in range(numGenerations):
        ...
        P1, fitnessScores = rouletteSelection(totalPool, totalScores, populationSize)
        rewardsHistory.append(max(totalScores))
        print(f"Generation {gen}: Best Reward = {max(totalScores)}")
    print("Optimization Complete.")
    
    best_idx = np.argmax(fitnessScores)
    evalfitness(P1[best_idx], render_mode="human")
    bestCheetah = P1[best_idx]
    numK = len(bestCheetah)

    with open("best_keyframes_tournament+roulette.txt", "w") as f:
        f.write(f"{numK}\n")
        for keyframe in bestCheetah:
            line = ", ".join([f"{val:.2f}" if i < 6 else f"{int(val)}" 
                             for i, val in enumerate(keyframe)])
            f.write(line + "\n")
    print("Best keyframes saved to 'best_keyframes_tournament+roulette.txt'!")
    ...
    plt.grid(True)
    plt.savefig("tournament+roulette.png")
    plt.show()
