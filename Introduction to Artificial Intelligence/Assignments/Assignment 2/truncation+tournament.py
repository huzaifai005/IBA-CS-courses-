...
def main():
    ...
    with open("best_keyframes_truncation+tournament.txt", "w") as f:
        f.write(f"{numK}\n")
        for keyframe in bestCheetah:
            line = ", ".join([f"{val:.2f}" if i < 6 else f"{int(val)}" 
                             for i, val in enumerate(keyframe)])
            f.write(line + "\n")
    print("Best keyframes saved to 'best_keyframes_truncation+tournament.txt'!")

    plt.figure(figsize=(10, 5))
    plt.plot(range(numGenerations), rewardsHistory, marker='o', color='r', linestyle='-')
    plt.title("Truncation Parents + Tournament Survivors")
    plt.xlabel("Generation")
    plt.ylabel("Best Episode Reward")
    plt.grid(True)
    plt.savefig("truncation+tournament.png")
    plt.show()
