...
def main():
    ...
    for gen in range(numGenerations):
        parents, parentScores = truncationSelection(P1, fitnessScores, numOffspring)       
        newChildren = []        
    ...
    with open("best_keyframes_truncation+roulette.txt", "w") as f:
        ...
        for keyframe in bestCheetah:
            ...
            f.write(line + "\n")
    print("Best keyframes saved to 'best_keyframes_truncation+roulette.txt'!")

    plt.figure(figsize=(10, 5))
    plt.plot(range(numGenerations), rewardsHistory, marker='o', color='orange', linestyle='-')
    plt.title("Truncation Parents + Roulette Survivors")
    plt.xlabel("Generation")
    plt.ylabel("Best Episode Reward")
    plt.grid(True)
    plt.savefig("truncation+roulette.png")
    plt.show()
