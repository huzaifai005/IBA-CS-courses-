...
def truncationSelection(pool, scores, numSelections):
    """Sorts and returns the top 'numSelections' individuals."""
...
def rouletteSelection(pool, scores, numSelections, replace=False):
    """Selects individuals based on proportional fitness. 
       replace=False ensures we don't pick the exact same clone twice for survivors."""
    minScore = min(scores)
...
def main():
    ...
    for gen in range(numGenerations):
        ...
        P1, fitnessScores = truncationSelection(totalPool, totalScores, populationSize)
        rewardsHistory.append(fitnessScores[0])
        print(f"Generation {gen}: Best Reward = {fitnessScores[0]}")

    print("Optimization Complete.")
    print("Showing the best cheetah animation...")
    evalfitness(P1[0], render_mode="human")
    bestCheetah = P1[0]
    numK = len(bestCheetah)

    with open("best_keyframes_tournament+truncation.txt", "w") as f:
        f.write(f"{numK}\n")
        for keyframe in bestCheetah:
            line = ", ".join([f"{val:.2f}" if i < 6 else f"{int(val)}" 
                             for i, val in enumerate(keyframe)])
            f.write(line + "\n")
    print("Best keyframes saved to 'best_keyframes_tournament+truncation.txt'!")

    plt.figure(figsize=(10, 5))
    plt.plot(range(numGenerations), rewardsHistory, marker='o', color='b', linestyle='-')
    plt.title("Evolutionary Algorithm Convergence (HalfCheetah-v5)")
    plt.xlabel("Generation")
    plt.ylabel("Best Episode Reward")
    plt.grid(True)
    plt.savefig("tournament+truncation.png")
    plt.show()
