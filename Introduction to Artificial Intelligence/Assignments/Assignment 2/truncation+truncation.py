...
def main():
    ...
    with open("best_keyframes_truncation+truncation.txt", "w") as f:
        ...
        for keyframe in bestCheetah:
            ...
            f.write(line + "\n")
    print("Best keyframes saved to 'best_keyframes_truncation+truncation.txt'!")

    plt.figure(figsize=(10, 5))
    plt.plot(range(numGenerations), rewardsHistory, marker='o', color='purple', linestyle='-')
    plt.title("Truncation Parents + Truncation Survivors (Pure Elitism)")
    plt.xlabel("Generation")
    plt.ylabel("Best Episode Reward")
    plt.grid(True)
    plt.savefig("truncation+truncation.png")
    plt.show()
